...
pub struct Vector2<T, const N: usize>(pub [T; N]) where [T; N]: LengthAtMost32;

/// Abstract vector over some finite field
pub trait Vector:
  Div<<Self as Vector>::Field, Output = Self>
  + Sub<Self, Output = Self>
  + Mul<<Self as Vector>::Field, Output = Self>
  + Copy {
  type Field: Float;
  ...
  /// Reflect self across vector across
  fn reflect(self, across: &Self) -> Self {
    // somehow I couldn't get two to compile so I just made it one twice lol
    self - (*across) * self.dot(&across) * (Self::Field::one() + Self::Field::one())
  }
  /// Refracts this vector across norm
  fn refract(self, norm: Self, eta: Self::Field) -> Option<Self> {
    let cos_l = self.dot(&norm);
    let discrim = Self::Field::one() - eta * eta * (Self::Field::one() - cos_l * cos_l);
    if discrim.is_sign_negative() {
      return None;
    }
    let cos_r = discrim.sqrt();
    // TODO check if the sign here might need to be flipped
...
pub fn bary_interp<T: Float, V: Vector<Field = T>>(weights: Vec3<T>, vals: Vec3<&V>) -> V
where
  V: Mul<T, Output = V> + Add<V, Output = V>, {
  let Vec3(&a, &b, &c) = vals;
  let Vec3(alpha, beta, gamma) = weights;
  assert!((alpha + beta + gamma).is_one());
...
impl<T: Float> Vec3<T> {
  ...
  pub fn clamp(&self, min: T, max: T) -> Vec3<T> { self.apply_fn(|v| v.max(min).min(max)) }
  #[inline]
...
impl<T: Float> Vec2<T> {
  ...
  /// Assuming the first two coordinates are valid barycentric coordinates
  /// Computes the third coordinate
  #[inline]
  pub fn barycentric(&self) -> T { T::one() - self.0 - self.1 }
  #[inline]
  pub fn is_valid_barycentric(&self) -> bool {
    T::zero() <= self.0 && T::zero() <= self.1 && T::one() >= self.0 + self.1
  }
  ...
  #[inline]
  pub fn apply_fn<F, S>(&self, f: F) -> Vec2<S>
  where
    F: Fn(T) -> S, {
